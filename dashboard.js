...
(() => {
  ...
  function renderPayments(items){
    ...
    body.innerHTML = items.map((p) => {
      const invoice = p.invoice_url ? (String(p.invoice_url).toLowerCase().includes(".pdf") ? `<a href="${p.invoice_url}" target="_blank" rel="noreferrer" download>Download</a>` : `<a href="${p.invoice_url}" target="_blank" rel="noreferrer">Open</a>`) : "—";
      return `<tr>
        <td>${fmtDate(p.created_at)}</td>
        <td>$${Number(p.amount_usd||0).toFixed(2)}</td>
        <td>${p.lypos||0} Credits</td>
        <td>${p.status||"—"}</td>
        <td>${invoice}</td>
      </tr>`;
    }).join("");
  ...
  async function maybeConfirmStripeReturn(){
    try{
      ...
      setMsg("Payment confirm error: " + (e.message || String(e)));
    }
  }
  async function load(){
    try{
