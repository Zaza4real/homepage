...
(() => {
  ...
  // Optimized message setter - NO FLICKERING
  let msgTimeout = null;
  function setMsg(msg) {
  ...
  async function doSignup() {
    ...
    try {
      ...
      if (typeof trackEvent === "function") {
        trackEvent("signup_failed", { error: e.message });
      }
      setMsg(`❌ ${e.message || "Could not create account. Please try again."}`);
    }
  }
  ...
  async function doLogin() {
    ...
    try {
      ...
      // Track login failure
      if (typeof trackEvent === "function") {
        trackEvent("login_failed", { method: "email", error: e.message });
      }
  ...
  async function handleGoogleSignIn(credential) {
    try {
      ...
      const out = await apiFetch("/api/auth/google", {
        method: "POST",
      ...
      if (typeof trackEvent === "function") {
        trackEvent("google_signin_failed", { error: e.message });
      }
      setMsg(`❌ ${e.message || "Google sign-in failed. Please try again."}`);
    }
  }
      ...
      passInput.addEventListener("input", () => {
        ...
        validationTimeout = setTimeout(() => {
          if (value.length > 0 && value.length < 6) {
            ...
            // Don't clear message if there's already an important message
            const currentMsg = $("authMsgPage")?.textContent || "";
            if (currentMsg.includes("⚠️ Password")) {
              setMsg("");
            }
          }
        }, 300); // 300ms debounce
