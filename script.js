...
function showAuthModal(show) {
  ...
  function setLoading(isLoading, text) {
    // Removed: setGenMsg(!!isLoading); - no longer showing "Generating" text at bottom
    ...
    if (pill) {
      ...
      pill.classList.toggle("active", !!isLoading); // Make status pill prominent
    }
    ...
    if (run) {
      ...
      run.classList.toggle("generating", !!isLoading); // Add breathing effect
    ...
    // Show/hide generating modal
    if (window.GeneratingModal) {
      if (isLoading) {
        window.GeneratingModal.show(
          "Translating Video...",
          "AI is dubbing and translating your video.<br>This may take several minutes."
        );
      } else {
  ...
  // ---- Professional generating status messages (shown in preview box)
  const GENERATING_MESSAGES = [
    "Processing your video...",
    "Analyzing audio and visuals...",
...
function setGenMsg(isGenerating){
  // REMOVED: No longer displays "Generating" message at bottom
  // Status is shown in the preview box header instead
  return;
}

// ========================================
// FIX MINI VIDEO PREVIEWS - FORCE LOAD FIRST FRAME
// ========================================

(function() {
  // Wait for page to load
  window.addEventListener('DOMContentLoaded', function() {
    const miniVideos = document.querySelectorAll('.miniVidEl');
    
    miniVideos.forEach(video => {
      if (!video) return;
      
      // Force load metadata to show first frame
      video.load();
      
      // Add loaded event to hide background icon when video is ready
      video.addEventListener('loadeddata', function() {
        const parent = video.closest('.miniVid');
        if (parent) {
          parent.style.setProperty('--video-loaded', '1');
          // Hide the background icon when video loads
          const style = document.createElement('style');
          style.textContent = `.miniVid[style*="--video-loaded"] ::before { opacity: 0; }`;
          if (!document.getElementById('miniVidLoadedStyle')) {
            style.id = 'miniVidLoadedStyle';
      ...
      video.style.backgroundColor = 'rgba(20, 20, 30, 0.8)';
    });
  });
})();
