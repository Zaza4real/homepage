/**
 * ✅ VOICEOVER BLOG POST UPLOADER
 * Paste this entire script into your browser console while logged in to dashboard
 * It will upload the new voiceover blog post!
 */

const BLOG_POST = {
  title: "Free AI Voiceover Tool That Actually Sounds Human",
  slug: "free-ai-voiceover-tool-human-quality",
  excerpt: "We just launched the fastest AI voiceover tool on the market. Generate professional voiceovers in seconds—completely free for new users. No credit card, no BS.",
  cover_url: "https://lypo.org/assets/voiceover-tool-launch-cover.jpg",
  video_url: "",
  status: "published",
  content_html: `
...
// Upload function
async function uploadVoiceoverBlog() {
  const token = localStorage.getItem("lypo_token_v1");
  if (!token) {
    console.error("❌ Not logged in! Please login to dashboard first.");
    return;
  }
  ...
  try {
    const response = await fetch(`${BACKEND_URL}/api/admin/blog/create`, {
      method: "POST",
      headers: {
    ...
    console.log("   URL: https://lypo.org/post.html?slug=" + result.post.slug);
    console.log("\n🎉 Done! Your blog post is now live!");
    
  } catch (err) {
    console.error("❌ Error uploading blog post:", err.message);
    console.log("\nTroubleshooting:");
    console.log("1. Make sure you're logged in to the dashboard");
    console.log("2. Make sure you have admin access");
    console.log("3. Check that the backend is running");
  }
}
