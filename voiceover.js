// voiceover.js - LYPO AI Voiceover Tool powered by Chatterbox Pro
...
(function () {
  ...
  const exaggerationSlider = $("exaggerationSlider");
  ...
  const newVoiceoverBtn = $("newVoiceoverBtn");
  ...
  function checkGenerateReady() {
    ...
    if (text.length > 0 && userBalance < cost) {
      ...
      if (referenceAudioUrl) {
        setStatus("Ready to generate with voice cloning", "ready");
      } else {
        setStatus("Ready to generate (default voice)", "ready");
      }
    }
  }
  ...
  async function checkAuth() {
    try {
      ...
      if (!token) {
        ...
        setStatus("Please login to generate voiceovers", "error");
        return;
      }
      ...
      if (creditsBalance) {
        creditsBalance.textContent = "Login required";
        creditsBalance.style.cursor = "pointer";
        creditsBalance.onclick = () => window.location.href = "auth.html";
      }
      generateBtn.disabled = true;
      setStatus("Please login to continue", "error");
    }
  }
  ...
  async function handleGenerate() {
    ...
    const temperature = parseFloat(temperatureSlider.value); // 0.1 to 1.0 (maps to cfg)
        ...
        if (data.status === "completed" && data.audioUrl) {
          ...
          if (data.status === "starting") {
            if (elapsed < 15) {
              ...
              statusMsg = `⏳ Initializing AI model... (${elapsed}s)`;
            } else if (elapsed < 60) {
              statusMsg = `⏳ Loading model - Almost ready... (${elapsed}s)`;
            } else {
              statusMsg = `⏳ Preparing (${elapsed}s) - First generation takes longer`;
            }
          } else if (data.status === "processing") {
            statusMsg = `⚡ Generating voiceover with Chatterbox Pro... (${elapsed}s)`;
          } else {
  ...
  function displayAudio(audioUrl) {
    ...
    downloadBtn.onclick = async (e) => {
      ...
      try {
        ...
        a.href = url;
        a.download = `lypo-voiceover-${Date.now()}.mp3`;
  ...
  function resetTool() {
    ...
    exaggerationValue.textContent = "0.5";
  ...
  function setStatus(message, type = "info") {
    ...
    if (type === "loading") {
      ...
      statusPill.querySelector(".ping").style.opacity = "0";
    } else if (type === "error") {
      ...
      statusPill.querySelector(".loader").style.opacity = "0";
      statusPill.querySelector(".ping").style.opacity = "1";
    } else {
      // ready/info state
      statusPill.querySelector(".loader").style.opacity = "0";
